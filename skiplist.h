...
public:
    ...
    // return a vector of node contain all the k-v between k1-k2
    vector<Node<K, V> *> range_search(K k1, K k2)const;
...
// TODO: finish range search
template<typename K, typename V>
vector<Node<K, V> *> SkipList<K, V>::range_search(K k1, K k2) const{
    vector<Node<K, V> *> range;
    Node<K, V> *node = header;
    for (int i = level; i >= 0; --i) {
        while ((node->forward[i])->key < k1) {
            node = *(node->forward + i);
        }
    }
    node = node->forward[0];
...
int SkipList<K, V>::getRandomLevel() {
    int level = static_cast<int>(rand()%MAX_LEVEL);
    if (level == 0) {
        level = 1;
    }
    return level;
